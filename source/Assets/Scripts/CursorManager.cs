...
public partial class CursorManager : Singleton<CursorManager>
{
    ...
    void LateUpdate()
    {
        if (GazingManager.Instance == null || CursorOnHolograms == null || CursorOffHolograms == null)
        {
            return;
        }
        ...
        this.gameObject.transform.position = GazingManager.Instance.Position + GazingManager.Instance.Normal * DistanceFromCollision;

        // Orient the cursor to match the surface being gazed at.
        gameObject.transform.up = GazingManager.Instance.Normal;
    }
}